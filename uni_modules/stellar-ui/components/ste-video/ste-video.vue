<template>
	<view class="ste-video-root" :class="cmpRootClass" :style="[cmpRootStyleVar]">
		...
		<video class="ste-video" :id="id" :src="videoSrc" :controls="false" :show-center-play-btn="false" :poster="poster"
			...
			@progress="progress" @loadeddata="loadeddata" @loadstart="loadstart" @seeked="seeked" @seeking="seeking"
			@loadedmetadata="loadedMetaData" @fullscreenclick="fullscreenclick">
			...
			<!-- 底部操作栏 -->
			<view class="cover bottom" :style="{ transform: showControl ? 'translateY(0)' : 'translateY(100%)' }"
				@click="isClickControl = true">
				<!-- 播放/暂停按钮 -->
			...
			<!-- 倍速、清晰度弹窗 -->
			<view class="popup-box" :style="{ transform: showPopup ? 'translateX(0)' : 'translateX(100%)' }"
				@click="handlePopupClick">
				<view class="item-box" :style="{ display: popupState == 1 ? 'flex' : 'none' }">
					<view class="choose-item" v-for="(e, index) in speedConfigArr" :key="index"
