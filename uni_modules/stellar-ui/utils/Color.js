...
class Color {
	...
	static hexa2rgba(hexa, a) {
		...
		const _a = a ? a : Number(parseInt(sColor.slice(6, 8), 16) / 255).toFixed(2)
		return `rgba(${result.join(",")},${ _a})`
	}

	static rgb2rgba(rgb, a = 1) {
		const result = [];
		for (let i = 1; i <= 3; i++) {
	...
	static setRgbaDiaphaneity(rgba, a) {
		const _rgb = rgba.replace(rgbaReg, "$1")
		const _a = a ? a : rgba.replace(rgbaReg, "$2")
		return `rgba(${_rgb},${_a})`;
	}

	static formatColor(color, a) {
