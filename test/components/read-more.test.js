...
import steReadMore from '../../uni_modules/stellar-ui/components/ste-read-more/ste-read-more.vue';

describe('Test ReadMore', () => {
	const moreStr =
	...
	test('test', () => {
		const wrapper = mount(
			{
				components: {
					'ste-read-more': steReadMore,
				},
				template: `<div class="more-block" style="height: 200px">
						<div class="title">演示效果</div>
						<ste-read-more showHeight="400"> ${moreStr} </ste-read-more>
					</div>`,
			},
			{
				propsData: {
		...
		const contentBoxEl = wrapper.find('.content-box');
